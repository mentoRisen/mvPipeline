...
| `reference_id` | A create-only, per-task integer slot (1, 2, 3…) on `Job`, unique within the task. Used to tie-break equal `order` values in API lists and Instagram publish ordering; not used by the worker pick order. AI draft bundles require every job to carry an explicit `reference_id`; runway `prompt.reference_id` must match an `imagecontent` sibling’s slot. | Distinct from display `order` and from the job UUID primary key. |
...
| `imagecontent` | A job `purpose` value meaning the job’s output is meant to become an image asset for publishing. | Used directly by `publisher_instagram.py` and UI job forms. Instagram publish preview still filters to this purpose only. |
| `videocontent` | A job `purpose` value for video-generation jobs (v1: paired with `runway-video` generator). | Process dispatch is wired via `processor_runway_video.py`; Instagram publish still image-only. |
| `runway-video` | Job `generator` token for Runway video jobs. Prompt JSON must include `prompt`, `model` (`gen4_turbo` or `veo3.1_fast`), and `reference_id` (image slot). | Case-insensitive at validation; stored as provided. Process requires referenced image slot to be `PROCESSED`. |
| `prompt.reference_id` | Integer inside job `prompt` JSON naming which **imagecontent** job slot on the same task supplies the still image. | Distinct from the job row’s own `reference_id` column (per-task slot id). UI labels use “Image slot” vs “Ref” for the row column. |
