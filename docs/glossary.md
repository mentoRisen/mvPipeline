...
| `videocontent` | A job `purpose` value for video-generation jobs (v1: paired with `runway-video` generator). | Processor dispatch for video is not wired yet; CRUD and authoring are supported. |
| `runway-video` | Job `generator` token for Runway video jobs. Prompt JSON must include `prompt`, `model` (`gen4_turbo` or `veo3.1_fast`), and `reference_id` (image slot). | Case-insensitive at validation; stored as provided. |
