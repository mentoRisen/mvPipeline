...
- Sequence of layers/components:
  ...
  AI create flow: `TaskList.vue` -> `AiTaskDraftModal.vue` -> `taskService.previewAiTaskDraft()` -> `POST /api/v1/tasks/ai-draft-preview` in `app/api/routes.py` -> `app/services/ai_task_draft_service.py` -> `app/services/integrations/llm_text_adapter.py`; each successful preview persists a row in `ai_draft_sessions` via `app/services/ai_draft_session_repo.py` and returns `draft_session_id`.
