...
- Sequence of layers/components:
  ...
  Runway video jobs (`purpose: videocontent`, `generator: runway-video`) use the job modal fields purpose, generator, model, image slot, and motion prompt. `prompt.reference_id` must match an `imagecontent` job on the same task; validation is shared in `app/services/job_prompt_validation.py` (REST and AI draft confirm). On **AI draft confirm**, every job must include an explicit row `reference_id`; runway `prompt.reference_id` is checked against those sibling slots before any insert (persist reuses the pre-validated IDs). **Process** is still enabled in the UI but `processor.py` does not implement `runway-video` yet (unknown generator error until a follow-up).
