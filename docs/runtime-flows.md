...
- Sequence of layers/components:
  ...
  AI confirm flow: `AiTaskDraftModal.vue` -> `taskService.confirmAiTaskDraft()` (optional `draft_session_id`) -> `POST /api/v1/tasks/ai-draft-confirm` -> `app/services/ai_task_draft_service.py` -> `task_repo.create_task_bundle_with_jobs()` (one transaction for the whole bundle); on success the session is marked completed, on validation/DB failure structured errors may be stored on the session for resume.
...
- Persistence touchpoints:
  tasks and jobs are stored in MySQL. Create/update/delete happens through a mix of repository functions and direct `Session(engine)` blocks in `app/api/routes.py`. AI preview persists an `ai_draft_sessions` row (brief + bundle JSON) per successful preview; AI confirm creates every task and job in the reviewed bundle through one shared backend workflow and **one** transaction (all-or-nothing).
- Integration touchpoints:
...
- Notable risks or inconsistencies:
  the home route uses local component state for selected task rather than the `/tasks/:id` route, so deep linking and in-app selection use different UI shells. The route layer still mixes HTTP handling, template shaping, and direct DB access in one file, although the AI draft preview/confirm path now moves multi-step draft logic into `app/services/ai_task_draft_service.py`. Draft sessions are **per authenticated user and tenant**; concurrent edits from multiple browser tabs are last-write-wins. Sessions expire after a configurable TTL (`AI_DRAFT_SESSION_TTL_DAYS` in `app/config.py`). There is still no LLM-based “repair” loop after failures (that remains a later slice). Deployments must ensure the `ai_draft_sessions` table exists (created via `SQLModel.metadata.create_all` on startup when models are registered).
...
- Sequence of layers/components:
  ...
  Runway video jobs (`purpose: videocontent`, `generator: runway-video`) use the job modal fields purpose, generator, model, image slot, and motion prompt. `prompt.reference_id` must match an `imagecontent` job on the same task; validation is shared in `app/services/job_prompt_validation.py` (REST and AI draft confirm). On **AI draft confirm**, every job must include an explicit row `reference_id`; runway `prompt.reference_id` is checked against those sibling slots before any insert (persist reuses the pre-validated IDs). **Process** for runway jobs resolves the referenced image slot at process time (`resolve_processed_image_slot` in `job_prompt_validation.py`): the sibling `imagecontent` job must be `PROCESSED` with a local `result.image_path`. TaskDetail disables **Process** until that slot is ready and shows a slot-specific reason. Dispatch goes through `processor.py` → `processor_runway_video.py` (Runway REST image-to-video, poll, download MP4 to `output/{task_id}/{job_id}.mp4`). Credentials: tenant `RUNWAY_API_KEY` first, then global `.env` (`resolve_runway_api_key`). Worker FIFO is unchanged — operators should process image jobs before video jobs manually.
